...
一个按工作日持续更新的中文编程学习网站。二十二条路线各自保持独立的课程序号和知识路径，每节课只聚焦一个主题，并提供最小代码示例、3～5 分钟练习与参考答案。
...
- 每个有效更新日，二十二条路线各推进一课；当天文件已存在时只校验，不重复生成。
...
app/                         网站页面与路由
components/                  通用界面组件
content/                     二十二条路线的 README 与课程 Markdown
...
仓库构建会验证 Markdown 能被网站读取并生成页面，但不会自动执行课程里的 KMP、ArkTS、Python、Rust、Flutter、Web、JVM、.NET、C++、Go、Android、Apple 平台或游戏开发工具示例。课程示例是否可运行，需要在对应语言、平台或引擎工程中另行验证。
